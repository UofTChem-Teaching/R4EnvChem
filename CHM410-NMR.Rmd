...
Install and load the `ChemoSpec` package as outlined in [Installing packages].
...
Next, we need to prepare the spectra for import into R. All the spectra in the **supplied dataset** contain `132,071` points. You will need to make sure your spectra also contains `132,071` data points. This can be done by zero-filling the spectra in MestreNova.

1. Select the **Processing** tab
2. Select **Zero Filling and LP**
3. Under the **To Spectrum Siz** dropdown menu, select **132071** and press **OK**
...
plotSpectra(spec,
            ...
            ##Set the x-limits of the plot. Notice how the limit is set from 10 to -1, since this is how NMR is commonly displayed.
            xlim = c(10, -1),
            ...
            ##The main plot title
            main = "Test NMR Spectra")
```

Note that some of these parameters look different then what you might be used to. This is because ChemoSpec does not use `ggplot()` for creating figures (yet). This is an example of one of the disadvantages of relying on third-party packages.
...
range <- spec$freq > -0.5 & spec$freq < 0.5

##Create a new Spectra object called norm that contains the normalized data
norm <- normSpectra(spec, method = "PQN", RangeExpress = range)

##Plot one of the original and one of the normalized spectra to compare. You should see a change in intensity.
plotSpectra(spec)
plotSpectra(norm)
```
...
plotScores(spec, 
           ...
           ## The title of the plot goes here
           main = "NMR PCA", 
