...
5. `remove = TRUE` removes the original sample column, to avoid duplication; setting this to `FALSE` would keep the original column. 
6. `convert = TRUE` converts the new columns to the appropriate data format. In the original column, the replicate number is a character value because it's part of a string, `convert` ensures that it'll be converted to a numerical value. 
