...
Let's plot our tided `atr_data` data:
...
Let's ignore the plot for now and look at our code down:
  ...
  2. We then specified we want to plot data from our `atr_data` dataset (`data = atr_data`).
  3. We then specified our *aesthetic mappings* via `aes()`. Here we'll pass information for how we want the plot to look. 
  4. To our aesthetic mappings we've specified which values from `atr_data` are supposed to be our x-axis values (`x = wavenumber`) and y-axis values (`y = absorbance`).
