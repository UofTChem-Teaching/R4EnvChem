...
ggplot(data = sumAtl,
  ...
  labs(x = "City in Atlantic Canada",
       y = "Mean NO<sub>2</sub> (ppb)")
```

Not an overly exciting plot, but it's gotten the job done showing the mean NO~2~ concentrations in each of the Atlantic Canada provinces (remember that this dataset was created in the [Summarizing Data] chapter). Let's break down the code above:
...
2. To our aesthetic mappings we've specified which values from the `sumAtl` are supposed to be our x-axis values (`x = city`) and y-axis values (`y = mean`).

3. We used `coord_flip()` to rotate our plot 90$^\circ$ therefore the supplied `x` option of `city` is now plotted on the `y-axis`. This makes reading long categorical names (i.e. the names of cities) easier. `coord_flip()` doesn't change anything else except the final orientation of the plot. 

4. We used `labs()` to provide clearer labels for our axes than those defined by the column titles. Note that because we flipped the axes the horzontal axis is now defined at the y-axis and the vertical axis is defined at the y-axis. Also note that the subscript in NO~2~ is defined using square brackets ([]) and superscript about be defined using a caret (^).


### Adding Error Bars

Almost all measurements have associated uncertainty/variability. These values are expressed visually via *error bars* demarcating the minimum and maximum variability and give a general idea of how precise a measurement is. In our `sumAtl` dataset in the [Further Summarize Operations] section we calculated the standard deviation of the mean NO~2~ concentration in each city as a measure of the variability around the mean. In our example, we've used the standard deviation (`sd`) as a measure of uncertainty of our calculated annual means.
