...
But managing these inputs and outputs is simplified with **variables**. Variables in R, like those you've encountered in math class, can only have one value, and you can reference or pass that value along by referring the variable name. And, unlike the variables in math classes, you can change that value whenever you want. Another way to think about it that a variable is a box in which you store your value. When you want to move (refecence) your value, you move the box (and whatevers inside of it). Then you can simply open the box somewhere else without having to worry about the hassle of what's inside. 
...
Now that you're familiar with navigating RStudio and some basic coding building blocks, you may have realized that working the console can get real messy, real quick. We'll read on to the next chapter where we'll discuss R workflows to make everyone's lives easier. 
