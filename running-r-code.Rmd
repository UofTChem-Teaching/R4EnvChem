...
As we saw last chapter, you can run code directly from the console. It's handy for short and sweet snippets of code, something that can be typed in a single line. Examples of this is the `install.packages()` function, or to use R as a calculator:
...
However, you'll soon run into issues when you write more complex code. That's why we write out code in *scripts*.

## Where to run code in RStudio

Scripts are a series of R commandes that will be executed one at a time, and are saved with the `.r` file extension. Let's open up a new script in RStudio by going to *File*->*New File*->*R Script*, or by clicking on the highlighted button in the image below.
...
![Figure 2.7: Find and replace button in the Scripts window of RStudio.](images/Rstudio_findreplace.png){width=100%}
