...
**Your Task:**
You will fit a linear model to investigate the relationship between `Standard Concentration` and `Analyte Peak Area (counts)`. Use `Standard Concentration` as the predictor (independent variable) and `Analyte Peak Area (counts)` as the response (dependent variable).

1. Ensure the data format is suitable for modeling.
...
Create a new column `peak_area_level` in `data` that categorizes the `analyte_peak_area_counts` into 'Low', 'Medium', or 'High', based on thresholds you define. Then, count how many occurrences fall into each category.
