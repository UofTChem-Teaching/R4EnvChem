...
1. Use the `read_csv()` function from the `readr`(`tidyverse`) package to import a CSV file named "sample_data.csv".  
...
2. What is the `age` in the third row?  
2. What is the `score` of `Name_5`?  
4. What is the `ID` of the last row?  
...
1. Convert the `limited_data` (which is a tibble) into a standard R dataframe.  
2. Assign this dataframe to a variable named `converted_data`.

```{r}
# Assume 'limited_data' is the Tibble from the previous exercise
...
# Exercise 4: Saving Data to Different Formats

**Your Task:**

1. Save the `converted_data` as a new CSV file named "saved_data.csv" using the `write_csv()` function from `tidyverse`.  
2. Save the same dataset as a `.txt` file named "saved_data.txt". Use a `tab` (or `\t` when you pass in as an argument) as the delimiter.

Note: Save it to the current working directory. 
**Consider**: For the `.txt` file, think about how you might specify a delimiter with the `write.table()` function. The sep argument could be useful here.
...
# Save data as a .txt file -- your code here
...
test_that("Saving Data to Different Formats", {
  ...
  expect_true(file.exists("saved_data.txt"))
  
  # Verify that the .txt file has the same number of lines as the dataset has rows (plus one for the header)
  expect_equal(length(readLines("saved_data.txt")), nrow(data_frame_version) + 1)
})
...
1. Use the `read_csv()` function to import only the first 10 rows of "sample_data.csv".  
...
test_that("Exploring Input Parameters of read_csv", {
  ...
  # Assuming that the first 2 rows of the original "sample_data.csv" contained unique values, 
  # ensure they are not present in the `limited_data` after skipping.
  original_first_row <- read_csv("sample_data.csv", n_max = 1)
  expect_false(original_first_row[1, 1] %in% limited_data[[1]])
  expect_false(original_first_row[1, 2] %in% limited_data[[2]])

  original_second_row <- read_csv("sample_data.csv", skip = 1, n_max = 1)
  expect_false(original_second_row[1, 1] %in% limited_data[[1]])
  expect_false(original_second_row[1, 2] %in% limited_data[[2]])
})
```
