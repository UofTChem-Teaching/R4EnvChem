---
title: 'Chapter 13: Transformation: Data Manipulation'
output:
  html_document:
    df_print: paged
---
...
**IMPORTANT**: Chapter 10 exercise is a prerequisite to this exercise. If you haven't done so yet, please complete chapter 10 exercises first. 
...
1. Before revisiting that, let's once again transform the `data` to a longer format where the headers `NO2` and `O3` become values in a new column called `analyte`, and their respective values fill a new column called `concentration`. Assign the result to `longer_data`.

2. Visualize the `Time`(x-axis) vs. `concentration`(y-axis) values as a scatterplot with `longer_data`.

```{r}
# Your code to make the data longer
longer_data <- 
...
3. Now, let's ensure the reliability of your data by removing potential measurement errors. Filter out any rows in data where the `concentration` values are less than 0. Update `longer_data` with the filter data.
...
**Your Task:** Now that you cleaned your data, re-visualize the `Time` and `concentration` values to confirm the effect of your data cleaning.

```{r}
# Your code for the scatterplot of Time vs concentration before cleaning goes here.
