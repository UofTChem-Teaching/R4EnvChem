...
- We use the `pivot_longer()` function to transform the data into a long format. This function combines the NO2 and O3 concentrations into a single column and creates a new column for the pollutant type. The transformation has already been completed for you, so the code should not be modified. You can learn more about this function in Chapter 15 of the textbook. The dataset you’ll be using is named `ox_long`.

```{r}
ox_long <- ox_data %>%
  pivot_longer(cols = c(NO2, O3),
               names_to = "Pollutant",
