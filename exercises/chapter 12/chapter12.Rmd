...
Similar to Chapter 11, we will transform the `data` to a longer format where the headers `NO2` and `O3` become values in a new column called `analyte`, and their respective values fill a new column called `concentration`. We assign the result to a variable named `longer_data`.
